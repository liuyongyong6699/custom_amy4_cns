...
xray_set() {
	...
	for opt in $xray_inbounds_options; do
		case $opt in
			...
			;;
			7)
				echo -n "请输入xray trojan tls端口: "
				read xray_trojan_tls_port
			;;
			8)
				echo -n "请输入xray trojan xtls端口: "
				read xray_trojan_xtls_port
			;;
		esac
	done
	echo -n "安装UPX压缩版本?[n]: "
	...
	export xray_install_directory xray_UPX xray_inbounds_options xray_http_port xray_ws_port xray_ws_path xray_ws_tls_port xray_ws_tls_path xray_mkcp_port xray_mkcp_xtls_port xray_tcp_xtls_port xray_trojan_tls_port xray_trojan_xtls_port
}
